...
def train_hsnn(args=None, wandb_flag=True):
    ...
    if args.use_test_as_valid:
        # be very careful. This selects the training set as the validation set. 
    ...
    @partial( jax.jit, static_argnames=['dilation', 'dilation_delta'] )
    def update(key, epoch, net_states, X, Y, opt_state, dropout_rate=0., dilation=5, dilation_delta=0):
        train_params = get_params(opt_state)
        # forward pass with gradients
        value, grads = value_and_grad(loss, has_aux=True, argnums=(1))(key, train_params, net_states, X, Y, epoch, dropout_rate=dropout_rate, dilation=dilation, dilation_delta=dilation_delta)
    ...
    def total_correct(net_params, net_states, X, Y, dilation=5, dilation_delta=0):
        args_in = [net_params, net_states, key, 0.]
        output_layer, _ = hsnn( args_in, X, dilation, dilation_delta )
        Yhat = decoder( output_layer )
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_dl):
            ...
            grads, opt_state, (L, [tot_correct, _]) = update(key, epoch, net_states, x, y, opt_state, dropout_rate=args.dropout_rate, dilation=args.conv_dilation, dilation_delta=args.delta_dil)
        ...
        for batch_idx, (x, y) in enumerate(val_dl):
            count_val += x.shape[0]
            acc_val += total_correct(net_params, net_states, x, y, args.conv_dilation, args.delta_dil)
    ...
    for batch_idx, (x, y) in enumerate(val_dl):
        ...
        _, net_states = params_initializer(key=key_epoch, args=args)
        count += x.shape[0]
        acc += total_correct(net_params_best, net_states, x, y, args.conv_dilation, args.delta_dil)
    # if args.dataset_name == 'mts_xor': count = count*args.nb_steps
